  ...
  <div class="rank-show">
    ...
    <ul class="ulTitle" :style="{'background-color':$c('#1b1b1b##排行榜列表头部背景颜色', __FILE__),color:$c('#ffffff##排行榜列表头部文本颜色', __FILE__)}">
      <li class="li-tit">
        ...
        <span class="sp-integral">当前总{{baseConfig.textcfg.jf_txt_tit}}</span>
      </li>
    </ul>
    <ul class="ulCon" style="position:relative;">
        <scroller>
      <li v-for="(item,index) in roomInfo.jfRankOption.jfRankList" :key="item.uid" :style="{color:$c('#ffffff##列表文本颜色', __FILE__)}">
        <span class="rank-tit">
        ...
        <span class="sp-integral" :style="{color:$c('#ffffff##积分文本颜色', __FILE__)}">{{item.jf_cur}}</span>
      </li>
        </scroller>
    </ul>
...
<script>
  ...
  export default {
    created() {
      this.lazyWatch('jfRankOption', 'roomInfo.active_menu', (newVal, oldVal) => newVal == 'RANKING_LIST')
    },
    methods: {
      load() {
      ...
      active() {
        return this.roomInfo.active_menu == 'RANKING_LIST' && this.roomInfo.active_rank == 'RANK_JF'
      },
      spIndBg(ind) {
        return {
