<template>
  <div class="rank-show">
    ...
    <ul class="ulCon" style="position:relative;">
      <scroller>
        <li v-for="(item,index) in roomInfo.giftGotRank.dataList" :key="item.uid" :style="{color:$c('#ffffff##列表文本颜色', __FILE__)}">
          <span class="rank-tit">
          ...
          <span class="sp-integral" :style="{color:$c('#ffffff##积分文本颜色', __FILE__)}">{{item.jf_got}}</span>
        </li>
      </scroller>
    </ul>
...
<script>
  ...
  export default {
    created() {
      this.lazyWatch('giftGotRank', 'roomInfo.active_menu', (newVal, oldVal) => newVal == 'RANKING_LIST')
    },
    methods: {
      load() {
