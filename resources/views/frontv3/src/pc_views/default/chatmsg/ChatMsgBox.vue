<template>
  <ul class="content">
    <li v-for="item in msgList" :key='item.id' class=" clearfix">
      <component :afterAppend="!roomInfo.screenLockStatus ?afterAppend : '' " :msgItemData=" item " :msgItemSty="getRoleType( item.isLuckMoney?item.user.role_id :item.role_id)" :is="msgType(item)" @click="selectMsg(item)"></component>
    </li>
  </ul>
...
<script>
  ...
  import ChatMsgItem from "@/pc_views/_/chatmsg/ChatMsgItem2";
  import SystemMsgItem from "@/pc_views/_/chatmsg/SystemMsgItem2";
  import PacketItem from "@/pc_views/_/chatmsg/PacketItem";
  ...
  export default {
    ...
    methods: {
      getRoleType(roleId) {
        var _styObj = {
          msgBgCo: $c("#fff7ec##默认消息的背景颜色", __FILE__),
        ...
        if (roleId > 0) {
          if (roleId >= 500) {
            _styObj = {
              msgBgCo: $c("#FFE7CE##管理员消息的背景颜色", __FILE__),
              msgFontCo: $c("#333333##管理员消息的字体颜色", __FILE__),
              msgNickCo: $c("#FF0000##管理员昵称的颜色", __FILE__),
              msgNickBgCo: $c("#000000##管理员昵称背景的颜色", __FILE__),
              msgFunBtnCo: $c("##管理员消息操作按钮的背景颜色", __FILE__),
            }
          } else if (roleId >= 400 && roleId < 500) {
            _styObj = {
              msgBgCo: $c("#fff7ec##讲师消息的背景颜色", __FILE__),
              msgFontCo: $c("#333333##讲师消息的字体颜色", __FILE__),
              msgNickCo: $c("#FF00FF##讲师昵称的颜色", __FILE__),
              msgNickBgCo: $c("#FF0000##讲师昵称背景的颜色", __FILE__),
              msgFunBtnCo: $c("##讲师消息操作按钮的背景颜色", __FILE__),
            }
          } else if (roleId == 100) {
            _styObj = {
              msgBgCo: $c("#fff7ec##游客消息的背景颜色", __FILE__),
            ...
            _styObj = {
              msgBgCo: $c("#fff7ec##会员消息的背景颜色", __FILE__),
