...
<head>
    ...
    @if(\app\api\ContentMgr::_content('pagecfg.icon', $room_id))
        <link href="{{ \app\api\ContentMgr::_content('pagecfg.icon', $room_id) }}" rel="shortcut icon"
              type="image/x-icon">
    @endif
    <meta name="Keywords" content="{{ \app\api\ContentMgr::_content('pagecfg.keywords', $room_id) }}" class="web-kword">
    <meta name="description" content="{{ \app\api\ContentMgr::_content('pagecfg.description', $room_id) }}" class="web-des">
...
<script type="text/javascript" src="{{$common_cdn}}/js/bootstrap-colorpalette/js/bootstrap-colorpalette.js?v={{$webver}}"></script>
...
<script type="text/javascript">
    ...
    var baseConfig = {
        ...
        caitiaocfg:{!! json_encode(\app\api\ContentMgr::_contentCfg('caitiaocfg', $room_id)) !!},
    ...
    function playVod(url) { //视频库 播放点播视频
        if(!(app.roomInfo.videoUrl && app.roomInfo.videoUrl.length)){
            app.$store && app.$store.commit("UPDATE_ROOM_INFO", {
                videoUrl: "/video?room_id=" + app.roomInfo.room_id + "&token=" + app.roomInfo.token + "&sta=1",
            });
        }
        if (videoWin.playVod) {
            videoWin.playVod(url);
        } else {
            setTimeout(function () {
                window.videoWin = document.getElementById('video-iframe').contentWindow;
                videoWin.playVod && videoWin.playVod(url);
            }, 500);
        }
    }
    ...
    function showVideo() {
        ...
        if ((baseConfig.channelInfo.needPwd && window.__checked_video_pwd__) || !baseConfig.channelInfo.needPwd) {
            ...
            if(window.baseConfig.channelInfo.vod_url && window.baseConfig.channelInfo.vod_url.length){
                playVod(window.baseConfig.channelInfo.vod_url);
            }else{
                if(!(app.roomInfo.videoUrl && app.roomInfo.videoUrl.length)){
    ...
    function startVideo() {
        if (!userInfo.lookvideo) {
            ...
            var _sta = app.roomInfo.videoUrl ? app.roomInfo.videoUrl : '';
            if (_sta.indexOf('sta') >= 0) {
                return;
            }
            showVideo();
        }
...
<script type="text/javascript" src="{{$cdn}}/assets/dist_f_dev/vendor.js?v=66414f2335fa1db7a65a_{{$webver }}"></script>
<script type="text/javascript" src="{{$cdn}}/assets/dist_f_dev/pc.js?v=66414f2335fa1db7a65a_{{$webver }}"></script>
<script type="text/javascript">
    $(function () {
