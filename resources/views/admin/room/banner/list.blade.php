...
<div class="row">
  <div class="col-sm-12">
    <section class="panel">
      <header class="panel-heading">房间图片列表</header>
        ...
        <div class="adv-table">
          <div class="clearfix" >
                    <div class="btn-group" style='margin-bottom:10px;'>
                      <a href="/admin/banner/banner" class="btn btn-primary">新增</a>
                    </div>
                </div>
      ...
      <tr>
        ...
        <!--<td>{{$list->url}}</td>-->
        <td>{{$list->sort}}</td>
        <td ><img src="{{$list->imgurl}}" width="30" height="30"></td>
        <td id='stateShow'>{{$flag[$list->flag]}}</td>
        <td>
        
           <form  style='float:left;margin-right:5px;'  method="post" action="/admin/banner/delete" onsubmit="return confirm('确定删除？');">
              {{ $ctrl->csrf_field() }}
              <input  type="hidden" name="id" value="{{$list->id}}">
              <button class="btn btn-sm btn-danger">删除</button>
            </form>
              <a class="btn btn-sm btn-primary" href="/admin/banner/banner?id={{$list->id}}">编辑</a>
        </td>
