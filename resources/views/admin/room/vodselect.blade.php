...
<div class="row">
  <div class="col-sm-12">
    <section class="panel">
      <header class="panel-heading">点播管理</header>
      <div class="panel-body">
        ...
        <div class="clearfix" >
            ...
            <form  action="/admin/room/vodselectedit" method="post">
       {{ $ctrl->csrf_field() }}
    ...
    @if($list->id > $i){{-- */$i=$list->id+1;/* --}}@endif
        <tr class="odd">
        <form id="editForm{{$list->id}}" action="/admin/room/vodselectedit" method="post">
       {{ $ctrl->csrf_field() }}
            ...
            <select id="m3u8_{{$list->id}}" style="width:100%" readonly name="m3u8"  class="editable form-control">
            {{-- */$selected=0;/* --}}
            <option >无</option>
              @foreach($vodList as $value)
              <option @if($vodCdn.'/'.$value->OssObject == $list->m3u8) {{-- */$selected = 1;/* --}}  selected @endif value="{{$vodCdn.'/'.$value->OssObject}}">{{date('m-d H:i',strtotime($value->StartTime))}}到{{date('m-d H:i',strtotime($value->EndTime))}}</option>
              @endforeach
              @if(!$selected && $list->m3u8)
            ...
            <td style="vertical-align: middle;">
            <a data-id="{{$list->id}}" class="btn btn-success edit" href="javascript:;">编辑</a>
            <a href="javascript:;" data-id="{{$list->id}}" class="btn btn-success btnpreview">预览</a></form>
            <form  action="/admin/room/vodselectdel" method="post">
              {{ $ctrl->csrf_field() }}
              <input type="hidden"  name="id" class="form-control " style="width:100%" value="{{$list->id}}">
