        ...
        <div class="adv-table">
          <div class="clearfix" >
            <div class="btn-group" style='margin-bottom:10px;'>
              <a href="/admin/proposeing/edit" class="btn btn-primary">新增</a>
            </div>
          </div>
          <div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline" role="grid">
            ...
            <table class="display table table-bordered table-striped dataTable" id="dynamic-table" aria-describedby="dynamic-table_info">
              ...
              <tbody role="alert" aria-live="polite" aria-relevant="all">
                ...
                <tr>
                  <td>{{$list->id}}</td>
                  <td>@if($list->teacher){{$list->teacher->name}}@else删除的讲师@endif</td>
                  <td>{{$list->title}}</td>
                  <td>{{$list->count}}</td>
                  @if($list->createUser)
                  ...
                  @endif
                  <td>
                    <form  style='float:left;margin-right:5px;'  method="post" action="/admin/proposeing/del" onsubmit="return confirm('确定删除？');">
                      {{ $ctrl->csrf_field() }}
                      <input  type="hidden" name="id" value="{{$list->
                      id}}">
                      <button class="btn btn-sm btn-danger">删除</button>
                    </form>
                    <a class="btn btn-sm btn-primary" href="/admin/proposeing/edit?id={{$list->id}}">编辑</a>
                  </td>
